-- danwa+調教 統合スコアV3 を行で返す(jra-card.ps1から@date/@venue/@loパラメータで呼ぶ)。
-- 返却: 開催場所,レース番号,馬番,馬名,印,v3,消し,矢印,追切class。
SET NOCOUNT ON;
...
SELECT 開催場所, レース番号, 馬番, 馬名, ISNULL(印,N'') AS 印, v3, ISNULL(消し,0) AS 消し, ISNULL(矢印,N'') AS 矢印, ISNULL(追切class,N'') AS 追切 FROM #r ORDER BY レース番号,馬番;
